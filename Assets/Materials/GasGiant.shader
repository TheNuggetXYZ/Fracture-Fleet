    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                float alpha = (gasDepth01 - _Fade) * (_Density);
                alpha = saturate(alpha);

                Light mainLight = GetMainLight();
                float3 lightDir = normalize(float3(0, 0, 0) - sphereCenter); //normalize(mainLight.direction);
                float3 lightColor = mainLight.color;

                float3 entryPos = rayOrigin + rayDirection * entry;
                float3 sphereCenterEntryDir = entryPos - sphereCenter;
                float lightFactor = dot(lightDir, normalize(sphereCenterEntryDir)) + 0.1;

                float3 col = _Color;
                float3 litColor = col * saturate(lightFactor);
